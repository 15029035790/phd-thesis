...
\long\def\BL{\textsc{Baseline}}
\long\def\LM{\textsc{Limit}}
\long\def\LMS{\textsc{Limit-SS}}
\long\def\LMC{\textsc{Limit-CB}}
...
%In \cite{trafficucb}, a participatory sensing framework with WST mode is proposed, whereas, in \cite{kazemi2011privacy}, a participatory sensing framework with SAT mode is introduced. 
...
%%Differential location privacy for sparse mobile crowdsensing~\cite{wang2016differential}
%
...
%A closely related topic of research is the privacy-preserving ride-sharing service~\cite{pham2017privateride}. This work designs a cryptographic protocol to protect riders' and drivers' identifiable information, including location.
...
%Geo-I provides privacy protection to a single individual's location data as opposed to aggregate information, which suits our SCGuard framework to be presented next.
...
%\STATE $\mathit{PSD_1}$ $\leftarrow$ Publish $PSD_1$ of $D_1$ with budget $\epsilon$\\
\STATE  Publish $PSD_1$ of $D_1$ with budget $\epsilon$\\
%\STATE $\mathit{PSD}$ $\leftarrow$ Adjust consistency of $\mathit{PSD}$ 
\end{algorithmic}
...
All other methods achieve close to the target $\mathit{EU}$ of $90\%$, but most often this is a result of over-provisioning, which in turn increases $ANW$.
...
\begin{comment}	
	...
	\end{minipage}
\end{comment}	
	\caption{Performance overhead for multiple-snapshot WorkerPSD when varying privacy budget $\varepsilon$.}
...
Note that pruning workers during U2U makes sense since it is the most time-consuming stage of SCGuard. %We do not include this runtime improvement in the experiments as it is not the focus of this paper.
